...
| 3   | extras            | one multiselect: `--ui tailwind\|none`, `--shadcn` / `--no-shadcn` (offered only for react, and picking it turns tailwind on), `--gsquery` / `--no-gsquery`, `--gws-emul` / `--no-gws-emul`; tailwind + shadcn preselected. A flag-decided row drops out of the list, and when every row is decided the prompt is skipped |
| 4   | git init, install | on by default; `--no-git`, `--no-install`                                                                                                                                                                                                                                                                                 |
| 5   | GAS project       | `--setup` / `--no-setup`; runs the generated project's `scripts/setup.mjs` after install; not asked when `--no-install`. Counted done only when the script exits 0 **and** wrote `envs.json` (picking "Later" exits 0), otherwise next steps print `<pm> run setup` and the hand-written `envs.json` shape                |
